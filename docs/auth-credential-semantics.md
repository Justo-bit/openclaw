...
Model selection can restart a stale read across the two publications of one OAuth
refresh: its claim and settlement. Each rejected read finishes cleanup before the
next read, preserving the selected agent and any explicit profile pin. Continued
changes beyond those two transitions, admission refusals, and cleanup failures
remain errors.
