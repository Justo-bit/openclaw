...
async function replay(
  ...
  const spawnSync = vi.fn((binary: string, args: string[]) => {
    ...
    if (binary.endsWith("lsof")) {
      ...
      return {
        status: 0,
        stdout: `p123\nfcwd\nn${runtimeDir.replace(/[^\x20-\x7e]/gu, "?")}${failure === "cwd" || (host && failure === "host-cwd") ? "-other" : ""}\n`,
      };
    }
    if (binary === "ps") {
      if (args.includes("lstart=")) {
        return {
          status: 0,
          stdout:
            failure === "missing-start" || (host && failure === "dead-host")
              ? ""
              : "Wed Sep  9 12:00:00 2026\n",
        };
      }
      ...
      if (args.includes("ppid=")) {
        return { status: 0, stdout: failure === "other-parent" ? "789" : "456" };
      }
      if (args.includes("uid=")) {
        return {
          status: 0,
          stdout:
            (host && failure === "host-uid") || (!host && failure === "node-uid") ? "0" : "501",
        };
      }
      if (host) {
        // C-locale ps is a display string, not a lossless argument transport.
        const hostCommand = hostArguments.join(" ").replace(/[^\x20-\x7e]/gu, "?");
        return { status: 0, stdout: failure === "host-command" ? "unrelated-host" : hostCommand };
      }
