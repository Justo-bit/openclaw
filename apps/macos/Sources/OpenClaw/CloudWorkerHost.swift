...
enum CloudWorkerHost {
    ...
    @MainActor private static func run(_ configuration: CloudWorkerHostConfiguration) async throws -> Int32 {
        ...
        let stopMonitoring = desktop.startMonitoring {
            if desktop.consoleState() != .unlocked {
                session.requestStop(reason: "the worker desktop was locked or its GUI session ended")
            }
        }
