...
export class DraftPreferenceState {
  ...
  synchronize() {
    ...
    const owner = [gateway, client, connected, recoveryScope, profileId];
    if (
      ...
      if (connected) {
        ...
        const listener = (agentId: string, preference: NewSessionPreference, consumed: boolean) => {
          ...
          if (consumed && this.read().agentsHydrated) {
  ...
  private preparePreferenceWrite(
    ...
    const publish = (preference: NewSessionPreference) => {
      ...
      for (const listener of writes.listeners) {
        listener(agentId, preference, accepted);
      }
    };
    const writeLocal = () => {
      ...
      const write = async () => {
        ...
        try {
          ...
          for (let attempt = 0; attempt < 3; attempt += 1) {
            ...
            // Route disposal cannot undo a committed value. Publish it to the
            // surviving owner-scoped projections without adopting another draft’s choices.
            if (!ownsConnection()) {
              return;
            }
